...
const pendingOpportunities = [
  {
    ...
    status: "under_review" as OpportunityStatus,
...
export function OpportunityOversight() {
  ...
  async function handleValidateAndScore(opp: typeof pendingOpportunities[0]) {
    // DRBE validation
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-3xl font-bold text-gray-900">Opportunity Oversight</h2>
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <div className="space-y-4">
            {pendingOpportunities.map((opp) => {
              const hasErrors = validationErrors[opp.id] && validationErrors[opp.id].length > 0;
              return (
                <div key={opp.id} className={`p-4 border rounded-lg mb-4 ${hasErrors ? 'border-red-500' : ''}`}>
